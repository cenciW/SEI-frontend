  ...
  return (
    <group position={position}>
      ...
      <mesh position={[0, 1, 0]} castShadow>
        ...
        <meshStandardMaterial color="#4a7c2a" roughness={0.8} metalness={0.1} />
      </mesh>
      ...
      {[
        ...
        <group key={i} position={[0, leaf.y, 0]} rotation={[0, leaf.rot, 0]}>
          <mesh
            position={[leaf.dir * 0.3, 0, 0]}
            rotation={[0, 0, leaf.dir * -0.4]}
            castShadow
          >
  ...
  const processedScene = React.useMemo(() => {
    ...
    clone.traverse((child: Object3D) => {
      if (child instanceof Mesh && child.material) {
        // Material pode ser um array — tratar isso corretamente
        ...
        if (Array.isArray(child.material)) {
          child.material.forEach((m) => {
            m.needsUpdate = true;
            if (m.color) m.color.needsUpdate = true;
            if (m.map) m.map.needsUpdate = true;
          });
        } else {
          child.material.needsUpdate = true;
          if (child.material.color) child.material.color.needsUpdate = true;
...
const CannabisPlant = ({
  ...
  // =========  FIELD (PLANTIO EM GRADE) =========
  if (mode === "field") {
    const gridPositions: [number, number, number][] = [];
    const spacing = 10;
    const gridSize = 1; // 1x1 (pode aumentar)
    const scale = 1;
  ...
  return (
    <group>
      ...
      <mesh position={[0, 0.0, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color="#a0522d" roughness={0.9} metalness={0.0} />
      </mesh>
      {/* Soil */}
      <mesh
        position={[0, 0.28, 0]}
...
export default function CropVisualization({
  crop,
  mode,
  ...
  const getPlant = () => {
    ...
    if (lowerCrop.includes("corn") || lowerCrop.includes("milho")) {
      ...
      lowerCrop.includes("cannabis") ||
      lowerCrop.includes("maconha")
    ) {
  ...
  const isCannabis =
    crop.toLowerCase().includes("cannabis") ||
    crop.toLowerCase().includes("maconha");
